...
//	gl_FragColor = vec4( v_Color.rgb * mono, mono );
	...
	float m_scanline_intensity = 0.25;
	...
	float monoin = (v_Color.r + v_Color.g + v_Color.b) / 3.0;
	float mono = (temp.r + temp.g + temp.b) / 3.0;
	if (mono < (monoin*0.25)) mono *= 0.5;
	if (monoin < 0.4) mono *= 3.0;
	temp = vec4( mono, mono, mono, temp.a ) * v_Color;
